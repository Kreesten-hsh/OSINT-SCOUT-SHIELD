...
import { ArrowLeft, CheckCircle2, Loader2, Mail, Phone, ShieldOff, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      <PageHero
        ...
        subtitle="Fiche complete de la PME, incidents relies et activite probatoire."
        eyebrow={
      ...
      <section className="grid gap-4 md:grid-cols-4 fade-rise-in-1">
        ...
        <article className="metric-card">
          <p className="metric-card-label">Derniere activite</p>
              ...
              <tbody>
                {data.recent_incidents.length === 0 ? (
                  ...
                  <tr key={item.incident_uuid} className="table-row">
                    ...
                    <td className="max-w-[280px]"><p className="line-clamp-2">{item.message_preview}</p></td>
            ...
            <tbody>
              {data.recent_reports.length === 0 ? (
                ...
                <tr key={item.report_uuid} className="table-row">
                  <td className="font-mono text-xs text-primary">{item.public_reference}</td>
                  <td className="max-w-[360px]"><p className="line-clamp-2">{item.message_preview}</p></td>
                  <td className="font-semibold">{item.risk_score}/100</td>
                  <td className="text-xs text-muted-foreground">{item.attachments_count}</td>
                  <td className="text-xs text-muted-foreground">{item.bundles_count}</td>
                </tr>
              ))}
            </tbody>
          </table>
