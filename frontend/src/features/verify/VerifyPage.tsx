...
export default function VerifyPage() {
  ...
  const [channel, setChannel] = useState<SignalChannel>('WEB_PORTAL');
  ...
  const submitReport = async () => {
    ...
    try {
      ...
      const response = await apiClient.post<APIResponse<IncidentReportData>>('/incidents/report', payload);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto w-full max-w-2xl space-y-6 px-4 py-8 md:py-12">
        <div className="rounded-xl border border-border bg-card p-6">
          <div className="mb-4 flex items-center gap-2">
            <ShieldAlert className="h-5 w-5 text-primary" />
            <h1 className="text-xl font-bold">Verification de message suspect</h1>
          </div>
          <p className="mb-6 text-sm text-muted-foreground">
            Canal citoyen (mobile/web) pour estimer le risque avant action.
          </p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium">Numero (optionnel)</label>
                <input
                  value={phone}
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
              >
                <option value="WEB_PORTAL">Interface web</option>
                <option value="MOBILE_APP">App mobile (PWA)</option>
              </select>
            </div>

            <button
              onClick={submitVerify}
              disabled={loading || reporting || message.trim().length < 5}
